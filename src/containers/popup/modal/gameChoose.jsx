import { appDispatch, popup_close, update_game_choose_subscription, useAppSelector } from "../../../backend/reducers";
import { Image } from "../../../components/shared/general";


export function gameChoose({ data }) {
	console.log(data);
	const gamesInSubscription = useAppSelector(state => state.globals.gamesInSubscription)
	const handleChooseGame = (gameId) => {
		appDispatch(update_game_choose_subscription({ planName: data.planName, volumeId: gameId }))
		appDispatch(popup_close())
	}
	return (
		<div className="max-w-[780] w-full h-auto p-[30px] rounded-lg self-center bg-[#222638]]">
			<h2 className="text-center">Chọn 1 game bạn muốn cài sẵn khi có máy</h2>
			...
			<div
				...
				{gamesInSubscription.map(item =>
					<div
						onClick={() => { handleChooseGame(item.volumeId) }}
						key={item.name}
