            ...
            <div key={index} className="sub">
              ...
              <ul className="list-none flex flex-col gap-[8px] px-[8px]">
                <li className="flex justify-between">
                  {" "}
                  <span className="inline-block min-w-[30px]">GPU:</span>{" "}
                  <span className="text-right">{sub.gpu}</span>
                </li>
                <li className="flex justify-between">
                  {" "}
                  <span className="inline-block min-w-[30px]">RAM:</span>{" "}
                  <span className="text-right">{sub.ram}</span>
                </li>
                <li className="flex justify-between">
                  {" "}
                  <span className="inline-block min-w-[30px]">Hours:</span>{" "}
                  <span className="text-right">{sub.hours}</span>
                </li>
                <li className="text-[10px] mt-[-8px]">
                  *giới hạn số giờ được sự dụng trong 1 {sub.for}.
                </li>
              </ul>
              <button
                className="mt-[24px] instbtn mx-auto handcr border-none h-[32px] !px-2"
                onClick={handlePayment({ type: sub.type, price: sub.price })}
              >
                Chuyển Khoản
              </button>
              {user?.id ? (
                      ...
                      return (
                        <PayPalButtons
                          ...
                          createSubscription={async (data, actions) =>
                            subscribe(user, sub.plan_id, actions)
                          }
                          onApprove={payment}
                          style={{
                            layout: "vertical",
                            shape: "pill",
                            color:
                              fundingSource == FUNDING.PAYLATER ? "gold" : "",
                          }}
                        />
                      );
                    })}
